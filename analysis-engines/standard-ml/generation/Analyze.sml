...
fun generateConstraints' prog pack nenv =
	...
	fun f_partlist _ = raise EH.TODO "no description, raised in the 'f_partlist' function of Analyze.sml"
	   ...
	   and f_exp indent (A.ExpAtExp atexp) =
		...
		f_atexp ((convertIndentToSpaces indent)^SS.bottomLeftCurve^SS.straightLine) atexp)
	     ...
	     | f_exp indent (A.ExpOp (st, id, labexp1, labexp2, _, lab, _)) =
		   ...
		   val a   = E.initValueIDAccessor (E.consAccessorId (I.ID (id, lab)) eqTypeVar ty (CL.consVAL ()) lab) lab
	       in
		   if st = "="
	   ...
	   and f_mrule indent (A.Mrule (labpat, labexp, _, lab, _)) =
		   ...
		   val (tv2, eqtv, cst2, contextSensitiveSyntaxError2) = f_labexp (indent^SS.bottomLeftCurve^SS.straightLine) labexp
		...
		* story for the equality type variable) because there is no new label *)
	     ...
	     | f_type indent (A.TypeTyCon (typseq, longtycon, _, lab, _)) =
		   ...
		   val indent = convertIndentToSpaces indent
		   val (sv, cst1, contextSensitiveSyntaxError1) = f_typeRow (indent^SS.verticalFork^SS.straightLine) typseq
		   ...
		   val equalityTypeConstraint = E.initEqualityTypeConstraint (T.consEQUALITY_TYPE_VAR eqTypeVar') (T.consEQUALITY_TYPE_VAR eqTypeVar) lab
	       in (tv, eqTypeVar', E.conscsts (lab, [c1, c2, c3, equalityTypeConstraint]) (E.unionConstraintsList [cst1, cst2]), contextSensitiveSyntaxError1)
	       end
