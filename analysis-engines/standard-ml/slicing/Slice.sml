...
fun printSlice' slprog indent sep =
	...
	fun sepLines NONE     _        _        _   = ""
	  | sepLines _        NONE     _        _   = ""
	  | sepLines (SOME l) (SOME k) NONE     ind = if l < k andalso sep
						      then "\n" ^ ind
						      else ""
	  | sepLines (SOME l) (SOME k) (SOME c) ind = if l < k andalso sep
						      then "\n" ^ ind ^ lengthToString c
						      else ""

	fun printProgs (A.Progs xs)                          ind = printProgList xs ind
	...
	and printProg (A.Prog xs)                            ind = #4 (printProgOneList xs ind)
	  | printProg (A.ProgDots pl)                        ind = ldots ^ #4 (printPartDots pl (pind ind)) ^ rdots
	...
	and printAFile (A.AFile (f, r, _, _))                ind =
	    (getLine [r], getLine [r], getCol [r], f)
	  | printAFile A.AFileDots                           _   =
	    (NONE, NONE, NONE, ldots ^ dots ^ rdots)

	and printABool (A.ABool (b, r, _, _))                ind =
	    (getLine [r], getLine [r], getCol [r], b)
	  | printABool A.ABoolDots                           _   =
	    (NONE, NONE, NONE, ldots ^ dots ^ rdots)

	and printProgOne (A.ProgOneDec td)                   ind = printTopDec td ind
	  | printProgOne (A.ProgOneExp (e, _, _, _, _))      ind =
	    let
		val (l, k, c, x) = printExp e ind
	    in (l, k, c, x ^ ";")
	    end
	  | printProgOne (A.ProgOneParse _)                  _   =
	    (NONE, NONE, NONE, ldots ^ "unparsable" ^ rdots)
	...
	and printFBO (A.FunBindO (i, j, si, se, _, _, _))    ind =
	  ...
	  | printFBO (A.FunBindOO (i, j, s, t, se, _, _, _)) ind =
	    let val (l, k, c, x) = printFunId     i  ind
		val (_, _, _, y) = printStrId     j  ind
	  ...
	  | printFBO (A.FunBindOT (i, j, s, t, se, _, _, _)) ind =
	    let val (l, k, c, x) = printFunId     i  ind
		val (_, _, _, y) = printStrId     j  ind
		val (_, _, _, z) = printLabSigExp s  ind
		val (_, _, _, w) = printLabSigExp t  ind
	  ...
	  | printFBO (A.FunBindOS (i, p, se, _, _, _))       ind =
	    let val (l, k, c, x) = printFunId     i  ind
		val (_, _, _, y) = printSpec      p  ind
		val (_, j, _, z) = printLabStrExp se ind
	    in (l, j, c, x ^ " (" ^ y ^ ") = "  ^ z)
	    end
	  | printFBO (A.FunBindOSO (i, p, si, se, _, _, _))  ind =
	    let val (l, k, c, x) = printFunId     i  ind
		val (_, _, _, y) = printSpec      p  ind
		val (_, _, _, z) = printLabSigExp si ind
		val (_, j, _, w) = printLabStrExp se ind
	  ...
	  | printFBO (A.FunBindOST (i, p, si, se, _, _, _))  ind =
	    let val (l, k, c, x) = printFunId     i  ind
		val (_, _, _, y) = printSpec      p  ind
		val (_, _, _, z) = printLabSigExp si ind
		val (_, j, _, w) = printLabStrExp se ind
	...
	and printSigDec (A.SigDec (sb, r, _))                ind =
	    ...
	    in (getLine [r], k, getCol [r], "signature " ^ x)
	    end
	...
	and printSigBindOne (A.SigBindOne (id, se, r, _, _)) ind =
	    ...
	    in (l1, k2', c1, x ^ sep1 ^ esep ^ "= " ^ sep2 ^ y)
	    end
	...
	and printStrDec (A.StrDec (xs, _, _))                ind = printStrDecOneList xs ind
	...
	and printStrDecOne (A.StrDecOneDec d)                ind =
	  ...
	  | printStrDecOne (A.StrDecOneStr (sb, r, _))       ind =
	    ...
	    in (getLine [r], k', getCol [r], "structure " ^ sep ^ x ^ ";") (* semicolon? *)
	    end
	  | printStrDecOne (A.StrDecOneFun (fb, r, _, _))    ind =
	    let val (l, k, c, x) = printFunBind fb ind
		val k'  = case k of NONE => getLine [r] | _ => k
		val sep = sepLines (getLine [r]) l c ind
	    in (getLine [r], k', getCol [r], "functor " ^ sep ^ x ^ ";") (* semicolon? *)
	    end
	  | printStrDecOne (A.StrDecOneLoc (s, t, rs, _, _)) ind =
	    let val (r1, r2, r3) =
		    case rs of
			[r1, r2, r3] => (r1, r2, r3)
	...
	and printStrBO (A.StrBindOneOp (id, i, e, rs, _, _))  ind =
	    let val (r1, r2) =
		    case rs of
			[r1, r2] => (r1, r2)
		      | _ => raise EH.DeadBranch "not the correct number of regions"
	  ...
	  | printStrBO (A.StrBindOneTr (id, i, e, rs, _, _))  ind =
	    let val (r1, r2) =
		    case rs of
			[r1, r2] => (r1, r2)
		      | _ => raise EH.DeadBranch "not the correct number of regions"
		...
		val sep3 = sepLines k2' (getLine [r2]) (getCol [r2]) ind
		val sep4 = sepLines (getLine [r2]) l3 c3 ind
	    in (l1, k3', c1, x ^ sep1 ^ " : " ^ sep2 ^ y ^ sep3 ^ " = " ^ sep4 ^ z)
	    end
	  | printStrBO (A.StrBindOne (id, se, r, _, _))      ind =
	    let val (l1, k1, c1, x) = printStrId     id ind
		val (l2, k2, c2, y) = printLabStrExp se ind
		val k2'  = case k2 of NONE => getLine [r] | _ => k2
		val sep1 = sepLines k1 (getLine [r]) (getCol [r]) ind
		val sep2 = sepLines (getLine [r]) l2 c2 ind
	    in (l1, k2', c1, x ^ sep1 ^ " = " ^ sep2 ^ y)
	    end
	...
	and printAstLTReaDO (A.LTReaDOne (d, t, rs, _, _))   ind =
	    let val (r1, r2) =
		    case rs of
			[r1, r2] => (r1, r2)
		      | _ => raise EH.DeadBranch "not the correct number of regions"
		...
		val sep2 = sepLines k1' (getLine [r2]) (getCol [r2]) ind
		val sep3 = sepLines (getLine [r2]) l2 c2 ind
	    in (l1, k2', c1, "type " ^ sep1 ^ x ^ sep2 ^ " = " ^ sep3 ^ y)
	    end
	...
	and printLTReaDesc (A.LTReaDesc (xs, _, _, _))         ind =
	...
	and printLabSigExp (A.LabSigExp (e, _, _, _, _))     ind = printSigExp e ind
	  | printLabSigExp (A.LabSigExpDots pl)              ind =
	    let val (l, k, c, x) = printPartDots pl (pind ind)
	    in (l, k, c, ldots ^ x ^ rdots)
	    end

	and printSigExp (A.SigExpBasic (sp, rs, _, _))       ind =
	    let val (r1, r2) =
		    case rs of
			[r1, r2] => (r1, r2)
		      | _ => raise EH.DeadBranch "not the correct number of regions"
	    ...
	    in (getLine [r1], getLine [r2], getCol [r1],
		"sig " ^ sep1 ^ x ^ sep2 ^ " end")
	    end
	  | printSigExp (A.SigExpId (id, _, _))              ind = printSigId id ind
	  | printSigExp (A.SigExpRea (se, rea, rs, _, _))    ind =
	    let val r =
		    case rs of
			[r] => r
		      | _ => raise EH.DeadBranch "not the correct number of regions"
	...
	and printStrExp (A.StrExpBasic (sd, rs, _, _))       ind =
	    let val (r1, r2) =
		    case rs of
			[r1, r2] => (r1, r2)
		      | _ => raise EH.DeadBranch "not the correct number of regions"
		...
		val sep1 = sepLines (getLine [r1]) l c ind
		val sep2 = sepLines k' (getLine [r2]) (getCol [r2]) ind
	    in (getLine [r1], getLine [r2], getCol [r1], "struct " ^ sep1 ^ x ^ sep2 ^ " end")
	    end
	  | printStrExp (A.StrExpId (id, _, _))              ind = printLongStrId id ind
	  | printStrExp (A.StrExpOp (se, si, _, _, _))       ind =
	    let val (l, k, c, x) = printLabStrExp se ind
		val (_, j, _, y) = printLabSigExp si ind
	    in (l, j, c, x ^ " :> " ^ y)
	    end
	  | printStrExp (A.StrExpTr (se, si, _, _, _))       ind =
	    let val (l, k, c, x) = printLabStrExp se ind
		val (_, j, _, y) = printLabSigExp si ind
	    in (l, j, c, x ^ " : " ^ y)
	    end
	  | printStrExp (A.StrExpFExp (id, se, _, _, _))     ind =
	    let val (l, k, c, x) = printFunId     id ind
		val (_, j, _, y) = printLabStrExp se ind
	    in (l, j, c, x  ^ "(" ^ y ^ ")")
	    end
	  | printStrExp (A.StrExpFDec (id, sd, _, _, _))     ind =
	    let val (l, k, c, x) = printFunId id ind
		val (_, j, _, y) = printStrDec sd ind
	    in (l, j, c, x  ^ "(" ^ y ^ ")")
	    end
	  | printStrExp (A.StrExpLocal (sd, se, rs, _, _))   ind =
	    let val (_, _, _, x) = printStrDec    sd ind
		val (_, _, _, y) = printLabStrExp se ind
	    in (getLine rs, getLine (rev rs), getCol rs, "let " ^ x ^ " in " ^ y ^ " end")
	    end
	...
	and printLongTyConEq (A.LongTyConEq (xs, rs, _, _))    ind =
	...
	and printSpecOne (A.SpecVal (vd, r, _, _))           ind =
	  ...
	  | printSpecOne (A.SpecExc (ed, r, _, _))           ind =
		...
		val k'  = case k of NONE => getLine [r] | _ => k
		val sep = sepLines (getLine [r]) l c ind
	    in (getLine [r], k', getCol [r], "exception " ^ sep ^ x)
	    end
	  | printSpecOne (A.SpecTdr (td, r, _, _))           ind =
	    let val (l, k, c, x) = printTdrDesc td ind
		val k'  = case k of NONE => getLine [r] | _ => k
		val sep = sepLines (getLine [r]) l c ind
	    in (getLine [r], k', getCol [r], "type " ^ sep ^ x)
	    end
	  | printSpecOne (A.SpecDat (dd, r, _, _))           ind =
	    let val (l, k, c, x) = printDatDesc dd ind
		val k'  = case k of NONE => getLine [r] | _ => k
		val sep = sepLines (getLine [r]) l c ind
	    in (getLine [r], k', getCol [r], "datatype " ^ sep ^ x)
	    end
	  | printSpecOne (A.SpecStr (sd, r, _, _))           ind =
	    ...
	    in (getLine [r], k', getCol [r], "structure " ^ sep ^ x)
	    end
	  | printSpecOne (A.SpecInc (si, r, _, _))           ind =
	    let val (l, k, c, x) = printLabSigExp si ind
		val k'  = case k of NONE => getLine [r] | _ => k
		val sep = sepLines (getLine [r]) l c ind
	    in (getLine [r], k', getCol [r], "include " ^ sep ^ x)
	    end
	  | printSpecOne (A.SpecIsi (si, r, _, _))           ind =
	    let val (l, k, c, x) = printSigIdSeq si ind
		val k'  = case k of NONE => getLine [r] | _ => k
		val sep = sepLines (getLine [r]) l c ind
	    in (getLine [r], k', getCol [r], "include " ^ sep ^ x)
	    end
	  | printSpecOne (A.SpecRep (tc, ltc, rs, _, _))     ind =
	    let val (r1, r2, r3) =
		    case rs of
			[r1, r2, r3] => (r1, r2, r3)
		      | _ => raise EH.DeadBranch "not the correct number of regions"
	    ...
	    in (getLine rs, k2', getCol rs,
		"datatype " ^ sep1 ^ x ^ sep2 ^ isep ^
		"= "        ^ sep3 ^ esep ^
		"datatype " ^ sep4 ^ y)
	    end
	  | printSpecOne (A.SpecSha (sp, tce, rs, _, _))     ind =
	    let val (r1, r2) =
		    case rs of
			[r1, r2] => (r1, r2)
		      | _ => raise EH.DeadBranch "not the correct number of regions"
	    ...
	    in (l1, k2', c1,
		x ^ sep1 ^ isep ^ "sharing type " ^ sep2 ^ y)
	    end
	  | printSpecOne (A.SpecSsi (sp, tsi, rs, _, _))     ind =
	    let val r =
		    case rs of
			[r] => r
		      | _ => raise EH.DeadBranch "not the correct number of regions"
	...
	and printStrDescOne (A.StrDescOne (id, se, r, _, _)) ind =
	    let val (l1, k1, c1, x) = printStrId     id ind
		val (l2, k2, c2, y) = printLabSigExp se ind
		val k2'  = case k2 of NONE => getLine [r] | _ => k2
		val sep1 = sepLines l1 (getLine [r]) (getCol [r]) ind
	...
	and printDatDescOne (A.DatDescOne (dn, cd, _, _, _)) ind =
	...
	and printConDescOne (A.ConDescOneId (id, _))         ind = printIdent id ind
	  | printConDescOne (A.ConDescOneOf (i, t, _, _, _)) ind =
	...
	and printValDescOne (A.ValDescOne (id, ty, _, _, _)) ind =
	...
	and printTypDescOne (A.TypDescOne (dn, _, _))        ind = printDatName dn ind
	...
	and printTdrDescOne (A.TdrDescOne (d, t, _, _, _))   ind =
	    let val (l, k, c, x) = printDatName d ind
		val (_, j, _, y) = printLabType t ind
	    in (l, j, c, x ^ " = " ^ y)
	    end
	...
	and printExcDescOne (A.ExcDescOne (id, _, _))        ind = printIdent id ind
	  | printExcDescOne (A.ExcDescOf (id, ty, _, _, _))  ind =
	    let val (l, k, c, x) = printLabId id ind
		val (_, j, _, y) = printLabType ty ind
	    in (l, j, c, x ^ " of " ^ y)
	    end
	...
	and printLongStrId (A.LongStrIdQual (a, b, _, _, _)) ind =
	...
	and printScon (A.SconInt    (s, _, r, _, _))         _   = (getLine [r], getLine [r], getCol [r], s)
	  | printScon (A.SconWord   (s, _, r, _, _))         _   = (getLine [r], getLine [r], getCol [r], s)
	  | printScon (A.SconReal   (s, _, r, _, _))         _   = (getLine [r], getLine [r], getCol [r], s)
	  | printScon (A.SconString (s, _, r, _, _))         _   = (getLine [r], getLine [r], getCol [r], String.translate transfun s)
	  | printScon (A.SconChar   (s, _, r, _, _))         _   = (getLine [r], getLine [r], getCol [r], String.translate transfun s)
	  | printScon A.SconDots                             _   = (NONE, NONE, NONE, ldots ^ dots ^ rdots)

	and printPcon (A.PconBool (s, _, r, _, _))           _   = (getLine [r], getLine [r], getCol [r], s)
	  | printPcon (A.PconRef  (s, _, r, _, _))           _   = (getLine [r], getLine [r], getCol [r], s)
	  | printPcon (A.PconNil  (s, _, r, _, _))           _   = (getLine [r], getLine [r], getCol [r], s)
	  | printPcon A.PconDots                             _   = (NONE, NONE, NONE, ldots ^ dots ^ rdots)

	and printLabId (A.LabId (id, _, _, _))               ind = printIdent id ind
	...
	and printLabClass (A.LabClass (cl, _, _, _))         ind = printClass cl ind
	  | printLabClass (A.LabClassDots pl)                ind =
	    let val (l, k, c, x) = printPartDots pl ind
	    in (l, k, c, ldots ^ x ^ rdots)
	    end

	and printClass (A.Class (s, _, r, _, _))             _   = (getLine [r], getLine [r], getCol [r], s)
	  | printClass A.ClassDots                           _   = (NONE, NONE, NONE, ldots ^ dots ^ rdots)

	and printStrId (A.StrId (s, _, r, _, _))             _   = (getLine [r], getLine [r], getCol [r], s)
	  | printStrId A.StrIdDots                           _   = (NONE, NONE, NONE, ldots ^ dots ^ rdots)

	and printSigId (A.SigId (s, _, r, _, _))             _   = (getLine [r], getLine [r], getCol [r], s)
	  | printSigId A.SigIdDots                           _   = (NONE, NONE, NONE, ldots ^ dots ^ rdots)

	and printFunId (A.FunId (s, _, r, _, _))             _   = (getLine [r], getLine [r], getCol [r], s)
	  | printFunId A.FunIdDots                           _   = (NONE, NONE, NONE, ldots ^ dots ^ rdots)

	and printLongId (A.LongIdQual (sid, lid, _, _, _))   ind =
	...
	and printLongTyCon (A.LongTyConQual (s, t, _, _, _)) ind =
	...
	and printTyLab (A.TyLab (s, r, _, _))                _   = (getLine [r], getLine [r], getCol [r], s)
	  | printTyLab (A.TyLabDots)                         _   = (NONE, NONE, NONE, ldots ^ dots ^ rdots)

	and printTyCon (A.TyCon (s, _, r, _, _))             _   = (getLine [r], getLine [r], getCol [r], s)
	...
	and printLabType (A.LabType (t, _, _, _))            ind = printType t ind
	...
	and printType (A.TypeOneVar (tv))                    ind = printTypeVar tv ind
	  | printType (A.TypeArrow (t1, t2, _, _, _))        ind =
	  ...
	  | printType (A.TypeParen (t, r1, r2, _, _))          ind =
	    let val (_, _, _, x) = printLabType t ind
	    in (getLine [r1], getLine [r2], getCol [r1], "(" ^ x ^ ")")
	    end
	...
	and printTyClass (A.TyClassCl (cl, r, _, _))         ind =
	    let val (l, k, c, x) = printLabClass cl ind
		val k'  = case k of NONE => getLine [r] | _ => k
		val sep = sepLines (getLine [r]) l c ind
	    in (getLine [r], k', getCol [r], "in " ^ sep ^ x)
	    end
	  | printTyClass (A.TyClassTy (ty, _, _))            ind = printType  ty ind
	  | printTyClass (A.TyClassDots pl)                  ind =
	    let val (l, k, c, x) = printPartDots pl (pind ind)
	    in (l, k, c, ldots ^ x ^ rdots)
	    end

	and printLabTyClass (A.LabTyClass (t, _, _, _))      ind = printTyClass t ind
	...
	and printTyClassSeq (A.TyClassSeqOne (t, _, _, _))   ind =
	  ...
	  | printTyClassSeq (A.TyClassSeqEm (r, _, _))       _   =
	    (getLine [r], getLine [r], getCol [r], splparen ^ sprparen) (*HACK: before it was: ""*)
	...
	and printConBind (A.ConBind (id, _))                 ind = printIdent id ind
	  | printConBind (A.ConBindOf (id, t, r, _, _))      ind =
		...
		val sep2 = sepLines (getLine [r]) l2 c2 ind
		val esep = case sep1 of "" => " " | _ => ""
	    in (l1, k2', c1, x ^ sep1 ^ esep ^ "of " ^ sep2 ^ y)
	    end
	...
	and printValBindCore (A.ValBindCore (p, e, r, _, _)) ind =
	...
	and printValBind (A.ValBindRec (vbs, r, _, _))       ind =
	    let val (l, k, c, x) = printValBindSeq vbs ind
		val k'  = case k of NONE => getLine [r] | _ => k
		val sep = sepLines (getLine [r]) l c ind
	    in (getLine [r], k', getCol [r], "rec " ^ sep ^ x)
	    end
	...
	and printLabAtPat (A.LabAtPat (ap, _, _, _))         ind = printAtPat ap ind
	...
	and printFMatch (A.FMatchId (id, _, _))              ind = printIdent id ind
	  | printFMatch (A.FMatchApp (fm, a, _, _, _, _))    ind =
	...
	and printLabFMatch (A.LabFMatch (fm, _, _, _))       ind =
	    let val (l, k, c, x) = printFMatch fm ind
	    in (l, k, c, x ^ " ")
	    end
	...
	and printFMatchTy (A.FMatchT fm)                     ind = printLabFMatch fm ind
	  | printFMatchTy (A.FMatchTTy (fm, ty, _, _, _))    ind =
	...
	and printFVBCore (A.FValBindCore (x, e, r, _, _))    ind =
	...
	and printFValBindOne (A.FValBindOne (x, _, _, _))    ind =
	...
	and printTypBind (A.TypBind (dn, ty, r, _, _))       ind =
	    let val (l1, k1, c1, x) = printDatName dn ind
	...
	and printExBind (A.ExBind (id, _, _))                ind = printIdent id ind
	  | printExBind (A.ExBindOf (id, t, _, _, _))        ind =
	    let val (l, k, c, x) = printLabId id ind
		val (_, j, _, y) = printLabType t ind
	    in (l, j, c, x ^ " of " ^ y)
	    end
	  | printExBind (A.ExBindEq (id, sid, _, _, _))      ind =
	    let val (l, k, c, x) = printLabId id ind
	...
	and printDec (A.DecVal (tvs, vb, r, _))              ind =
	  ...
	  | printDec (A.DecFVal (tvs, fvb, r, _))            ind =
		...
		val k2' = case k2 of NONE => k1 | _ => k2
		val sep = sepLines (getLine [r]) l1 c1 ind
	    in (getLine [r], k2', getCol [r], "fun " ^ sep ^ x ^ " " ^ y)
	    end
	  ...
	  | printDec (A.DecDatWith (db, tb, rs, _, _))       ind =
	    let val (_, _, _, x) = printDatBindSeq db ind
		val (_, j, _, y) = printTypBindSeq tb ind
	    in (getLine rs, j, getCol rs, "datatype " ^ x ^ " withtype " ^ y)
	    end
	  | printDec (A.DecDatRep (tc, ltc, rs, _, _))       ind =
	    let val (_, _, _, x) = printTyCon tc ind
		val (_, j, _, y) = printLongTyCon ltc ind
	    in (getLine rs, j, getCol rs, "datatype " ^ x ^ " = datatype " ^ y)
	    end
	  | printDec (A.DecType (tbs, r, _))                 ind =
	    let val (_, k, _, x) = printTypBindSeq tbs ind
	    in (getLine [r], k, getCol [r], "type " ^ x)
	    end
	  | printDec (A.DecEx (ebs, r, _))                   ind =
	    let val (_, k, _, x) = printExBindSeq ebs ind
	    in (getLine [r], k, getCol [r], "exception " ^ x)
	    end
	  | printDec (A.DecOpen (ids, r, _, _))              ind =
	    let val (l, k, c, x) = printLongStrSeq ids ind
		val k'  = case k of NONE => getLine [r] | _ => k
		val sep = sepLines (getLine [r]) l c ind
	    in (getLine [r], k', getCol [r], "open " ^ sep ^ x)
	    end
	  | printDec (A.DecLocal (d1, d2, rs, _, _))         ind =
	    let val (_, _, _, x) = printDecs d1 ind
		val (_, _, _, y) = printDecs d2 ind
	    in (getLine rs, getLine (rev rs), getCol rs, "local " ^ x ^ " in " ^ y ^ " end")
	    end
	  | printDec (A.DecAbsType (db, ds, rs, _, _))       ind =
	    let val (_, _, _, x) = printDatBindSeq db ind
		val (_, _, _, y) = printDecs ds ind
	    in (getLine rs, getLine (rev rs), getCol rs, "abstype " ^ x ^ " with " ^ y ^ " end")
	    end
	  | printDec (A.DecAbsWith (db, tb, ds, rs, _, _))   ind =
	    let val (_, _, _, x) = printDatBindSeq db ind
		val (_, _, _, y) = printTypBindSeq tb ind
		val (_, _, _, z) = printDecs ds ind
	    in (getLine rs, getLine (rev rs), getCol rs, "abstype " ^ x ^ " withtype " ^ y ^ " with " ^ z ^ " end")
	    end
	  | printDec (A.DecInfix (i, ids, r, _, _))          ind =
	    let val (_, k, _, x) = printIdentSeq ids ind
	    in (getLine [r], k, getCol [r], "infix " ^ x)
	    end
	  | printDec (A.DecInfixr (i, ids, r, _, _))         ind =
	    let val (_, k, _, x) = printIdentSeq ids ind
	    in (getLine [r], k, getCol [r], "infixr " ^ x)
	    end
	  | printDec (A.DecNonfix (ids, r, _, _))            ind =
	    let val (_, k, _, x) = printIdentSeq ids ind
	    in (getLine [r], k, getCol [r], "nonfix " ^ x)
	    end
	  | printDec (A.DecOverload (i, t, v, s, rs, _, _))  ind =
	    ...
	    in (getLine [r1], k4', getCol [r1],
		"overload " ^ sep1 ^ x ^ sep2 ^ asep ^
		": "        ^ sep3 ^ y ^ sep4 ^ bsep ^
		"with "     ^ sep5 ^ z ^ sep6 ^ csep ^
		"in "       ^ sep7 ^ w)
	    end
	  | printDec (A.DecClass (i, s, r, _, _))            ind =
		...
		val asep = case sep2 of "" => " " | _ => ""
	    in (getLine [r], k2', getCol [r],
		"overload " ^ sep1 ^ x ^ sep2 ^ asep ^ y)
	    end
	...
	and printLabExp (A.LabExp (e, _, _, _, _))           ind = printExp e ind
	...
	and printSeqExp (A.SeqExp (el, e, _, _, _, _))       ind = printLabExpSeq (el @ [e]) ind
	  | printSeqExp (A.SeqExpSl (pl, e, _, _, _))        ind =
	    let val (l, k, c, x) = printPartDots pl ind
		val (_, j, _, y) = printLabExp e ind
	    in (l, j, c, x ^ ";" ^ y)
	    end
	...
	and printAtExp (A.AtExpId id)                        ind = printLongId id ind
	  ...
	  | printAtExp (A.AtExpTuple (el, rs, _, _))         ind =
	  ...
	  | printAtExp (A.AtExpLet (d, e, rs, _, _))         ind =
		...
		val isep = case sep2 of "" => " " | _ => ""
		val esep = case sep4 of "" => " " | _ => ""
	    in (getLine [r1], getLine [r3], getCol [r1],
		"let " ^ sep1 ^ x ^ sep2 ^ isep ^
		"in "  ^ sep3 ^ y ^ sep4 ^ esep ^
		"end")
	    end
	  | printAtExp (A.AtExpDLet (d, s, rs, _, _))        ind =
	    let val (r1, r2, r3) =
		    case rs of
			[r1, r2, r3] => (r1, r2, r3)
		      | _ => raise EH.DeadBranch "not the correct number of regions"
	  ...
	  | printAtExp (A.AtExpParen (e, r1, r2, _, _))      ind =
	    let val (_, _, _, x) = printLabExp e ind
	    in (getLine [r1], getLine [r2], getCol [r1], "(" ^ x ^ ")")
	    end
	  | printAtExp (A.AtExpList (el, rs, _, _))          ind =
	    let val (_, _, _, x) = printLabExplist el ind
	    in (getLine rs, getLine (rev rs), getCol rs, "[" ^ x ^ "]")
	    end
	  | printAtExp (A.AtExpProj (tl, r, _, _, _))        ind =
	    let val (_, k, _, x) = printTyLab tl ind
	    in (getLine [r], k, getCol [r], "#" ^ x)
	    end
	  | printAtExp (A.AtExpSeq (seq, rs, _, _))          ind =
	    let val (_, _, _, x) = printSeqExp seq ind
	    in (getLine rs, getLine (rev rs), getCol rs, "(" ^ x ^ ")")
	    end
	  | printAtExp (A.AtExpQuote (quotes, regs, _, _))   ind =
	    let val (_, _, _, x) = printQuotes quotes ind
	    in 	(getLine regs, getLine (rev regs), getCol regs, "`" ^ x ^ "`" )
	    end
	...
	and printQuote (A.Quote (str, reg, _, _))            _   =
	    (getLine [reg], getLine [reg], getCol [reg], str)
	  | printQuote (A.Antiquote (exp, [reg], _, _))      ind = (* means it's an identifier *)
	    let val (_, k, _, x) = printExp exp ind
	    in (getLine [reg], k, getCol [reg], "^" ^ x)
	    end
	  | printQuote (A.Antiquote (exp, regs, _, _))       ind = (* otherwise it's an expression in parentheses *)
	    let val (_, k, _, x) = printExp exp ind
	    in (getLine regs, k, getCol regs, "^(" ^ x ^ ")")
	    end
	...
	and printExp (A.ExpAtExp ae)                         ind = printAtExp ae ind
	  | printExp (A.ExpFn (m, r, _, _))                  ind =
	    let val (_, k, _, x) = printMatch m ind
	    in (getLine [r], k, getCol [r], "fn " ^ x)
	    end
	  | printExp (A.ExpApp (e, ae, _, _, _, _, _))       ind =
	    let val (l, k, c, x) = printExp e ind
		val (_, j, _, y) = printAtExp ae ind
	    in (l, j, c, x ^ " " ^ y)
	    end
	  | printExp (A.ExpCase (e, m, r1, r2, _, _))        ind =
	  ...
	  | printExp (A.ExpConsList (_, e1, e2, _, _, _))    ind =
	    let val (l, k, c, x) = printLabExp e1 ind
		val (_, j, _, y) = printLabExp e2 ind
	    in (l, j, c, x ^ " :: " ^ y)
	    end
	  | printExp (A.ExpOp (s, _, e1, e2, _, _, _))       ind =
	    let val (l, k, c, x) = printLabExp e1 ind
		val (_, j, _, y) = printLabExp e2 ind
	    in (l, j, c, x ^ " " ^ s ^ " " ^ y)
	    end
	  | printExp (A.ExpOr (e1, e2, _, _, _))             ind =
	    let val (l, k, c, x) = printLabExp e1 ind
		val (_, j, _, y) = printLabExp e2 ind
	    in (l, j, c, x ^ " orelse " ^ y)
	    end
	  | printExp (A.ExpAnd (e1, e2, _, _, _))            ind =
	    let val (l, k, c, x) = printLabExp e1 ind
		val (_, j, _, y) = printLabExp e2 ind
	    in (l, j, c, x ^ " andalso " ^ y)
	    end
	  | printExp (A.ExpTyped (e, t, _, _, _))            ind =
	    let val (l, k, c, x) = printLabExp e ind
		val (_, j, _, y) = printLabType t ind
	    in (l, j, c, x ^ " : " ^ y)
	    end
	  | printExp (A.ExpIte (e, f, g, rs, _, _))          ind =
	    let val (r1, r2, r3) =
		    case rs of
			[r1, r2, r3] => (r1, r2, r3)
		      | _ => raise EH.DeadBranch "not the correct number of regions"
	  ...
	  | printExp (A.ExpWhile (e1, e2, r1, r2, _, _))     ind =
	  ...
	  | printExp (A.ExpRaise (e, r, _, _))               ind =
	    let val (_, k, _, x) = printLabExp e ind
	    in (getLine [r], k, getCol [r], "raise " ^ x)
	    end
	  | printExp (A.ExpHandle (e, m, _, _, _))           ind =
	    let val (l1, k1, c1, x) = printLabExp e ind
		val (_,  k2, _,  y) = printMatch m ind
	    in (l1, k2, c1, x ^ " handle " ^ y)
	    end
	...
	and printMrule (A.Mrule (p, e, r, _, _))             ind =
	    let val (l1, k1, c1, x) = printLabPat p ind
		val (l2, k2, c2, y) = printLabExp e ind
		val sep1 = sepLines k1 (getLine [r]) (getCol [r]) ind
		val sep2 = sepLines (getLine [r]) l2 c2 ind
		val asep = case sep1 of "" => " " | _ => ""
	    in (l1, k2, c1, x ^ sep1 ^ asep ^ "=> " ^ sep2 ^ y)
	    end
	...
	and printLabPat (A.LabPat (p, _, _, _, _))           ind = printPat p ind
	...
	and printAtPat (A.AtPatWild (r, _))                  _   = (getLine [r], getLine [r], getCol [r], "_")
	  ...
	  | printAtPat (A.AtPatTuple (pl, rs, _, _))         ind =
	  ...
	  | printAtPat (A.AtPatList (pl, rs, _, _))          ind =
	    let val (_, _, _, x) = printLabPatList pl ind
	    in (getLine rs, getLine (rev rs), getCol rs, "[" ^ x ^ "]")
	    end
	  | printAtPat (A.AtPatOr (xs, rs, _, _))            ind =
	...
	and printLabIdTy (A.LabIdTy (id, _, _, _))           ind = printIdentTy id ind
	...
	and printPat (A.PatAtPat ap)                         ind = printAtPat ap ind
	  | printPat (A.PatApp (id, ap, _, _, _, _))         ind =
	    let val (l, k, c, x) = printLongId id ind
		val (_, j, _, y) = printAtPat ap ind
	    in (l, j, c, x ^ " " ^ y)
	    end
	  | printPat (A.PatConsList (_, p1, p2, _, _, _))    ind =
	    let val (l, k, c, x) = printLabPat p1 ind
		val (_, j, _, y) = printLabPat p2 ind
	    in (l, j, c, x ^ " :: " ^ y)
	    end
	  | printPat (A.PatOp (st, _, p1, p2, _, _, _))      ind =
	    let val (l, k, c, x) = printLabPat p1 ind
		val (_, j, _, y) = printLabPat p2 ind
	    in (l, j, c, x ^ " " ^ st ^ " " ^ y)
	    end
	  | printPat (A.PatTyped (p, t, _, _, _))            ind =
	    let val (l, k, c, x) = printLabPat p ind
		val (_, j, _, y) = printLabType t ind
	    in (l, j, c, x ^ " : " ^ y)
	    end
	  | printPat (A.PatAs (id, p, _, _, _))              ind =
	    let val (l, k, c, x) = printLabIdTy id ind
		val (_, j, _, y) = printLabPat p ind
	    in (l, j, c, x ^ " as " ^ y)
	    end
	  ...
	  let
		...
		| sl_declist (d :: dl) ll =
		  let val nxt = A.getDecNext d
		      val (lll, llr) = splitList nxt ll
		      (*val _ = Debug.printdebug2 ("-+-" ^ (L.toString lll) ^ "--" ^ (L.printelt nxt) ^ "--" ^ (L.toString llr) ^ "\n")*)
		      val sld = sl_sl_dec d lll
		      (*val _ = Debug.printdebug2 ("--" ^ (printSlDec sld) ^ "\n")*)
		      val sldl = sl_sl_declist dl llr
		      (*val _ = Debug.printdebug2 ("---" ^ (printSlDecList sldl) ^ "\n")*)
		  in sld :: sldl
		  end
		...
		| sl_atexp (A.AtExpLet (ds, e, rl, l, n)) ll =
		  ...
		  then let
			  ...
			  (*val _ = Debug.printdebug2 ("---" ^ (L.toString llr))*)
			  val sle = sl_sl_labexp e llr
			  (*val _ = Debug.printdebug2 ("---" ^ (printSlLabExp sle))*)
		...
		| sl_strdeconelist (x :: xs) ll =
		  let val nxt = A.getStrDecOneNext x
		      val (ll1, ll2) = splitList nxt ll
		      (*val _ = Debug.printdebug2 ("--" ^ (L.toString ll1) ^ "--" ^ (L.printelt nxt) ^ "--" ^ (L.toString ll2) ^ "\n")*)
		...
		| sl_strbindonelist (x :: xs) ll =
		  ...
		  then let val nxt1 = A.getStrIdNext id
		       ...
		       (*val _ = Debug.printdebug2 ("----" ^ (printSlStrExp slse) ^ "\n")*)
		...
		| sl_sl_proglist ((x, y, b, n) :: xs) ll =
	      ...
	      (*val _ = Debug.printdebug2 ("printing result of L.delete L.dummyLab labels from function 'slice'...\n")*)
	      val labels' = L.delete L.dummyLab labels
	      (*val _  = Debug.printdebug2 ("slicing..." ^ L.toString labels')*)

	      val slice = sl_progs prog labels'

	      (*val _ = Debug.printdebug2 (toString slice)*)
	      (*val _ = Debug.printdebug2 (PP.prettyPrintAst slice)*)
