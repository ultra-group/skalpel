...
fun parseTest testfile =
	...
	fun getErrors (JSON.ARRAY []) = (D.printDebug 1 D.JSON "finished parsing errors object!"; [])
	  | getErrors (JSON.ARRAY(h::t)) =
	    let
		val _   = D.printDebug 3 D.JSON "parsing errors object. Getting a new error..."
	...
	val _ = D.printDebug 3 D.JSON ("getting 'errors' object...")
	val (errorList, test) = getObject test "errors"
	val _ = D.printDebug 3 D.JSON ("getting 'time' object...")
	val (timeObj, test) = getObject test "time"
	val _ = D.printDebug 3 D.JSON ("getting 'tyvar' object...")
	val (tyvarObj, test) = getObject test "tyvar"
	val _ = D.printDebug 3 D.JSON ("getting 'ident' object...")
	val (ident, test) = getObject test "ident"
	val _ = D.printDebug 3 D.JSON ("getting 'constraint' object...")
	val (constraintObj, test) = getObject test "constraint"
	val _ = D.printDebug 3 D.JSON ("getting 'labels' object...")
	val (labels, test) = getObject test "labels"
	val _ = D.printDebug 3 D.JSON ("getting 'minimisation' object...")
	val (minimisation, test) = getObject test "minimisation"
	val _ = D.printDebug 3 D.JSON ("getting 'solution' object...")
	val (solution, test) = getObject test "solution"
	val _ = D.printDebug 3 D.JSON ("getting 'basis' object...")
	val (basis, test) = getObject test "basis"
	val _ = D.printDebug 3 D.JSON ("getting 'timelimit' object...")
	val (timelimit, test) = getObject test "timelimit"
	val _ = D.printDebug 3 D.JSON ("getting 'labelling' object...")
	val (labelling, test) = getObject test "labelling"
	val _ = D.printDebug 3 D.JSON ("getting final object...")
	val (final, test) = getObject test "final"
	val _ = D.printDebug 3 D.JSON ("getting 'name' object...")
	val (name, test) = getObject test "name"
	val _ = D.printDebug 1 D.JSON ("top level json objects correct!")
	val _ = D.printDebug 2 D.JSON ("parsing lower level objects...")
	val _ = D.printDebug 3 D.JSON ("getting all errors...")
	val errs = getErrors errorList
	val _ = D.printDebug 3 D.JSON ("skipping time, will get that at the end...")
	val _ = D.printDebug 3 D.JSON ("getting tyvar...")
	val tv = (getInt(findIdVal tyvarObj "tyvar"), getTyvars(findIdVal tyvarObj "assoc"))
	val _ = D.printDebug 3 D.JSON ("getting ident...")
	val idnt = getTyvars(ident)
	val _ = D.printDebug 3 D.JSON ("getting constraint...")
	...
	val _ = D.printDebug 3 D.JSON ("getting labels...")
	val lab = getInt(labels)
	val _ = D.printDebug 3 D.JSON ("getting minimisation...")
	val min = getBool(minimisation)
	val _ = D.printDebug 3 D.JSON ("getting solution...")
	val sol = getInt(solution)
	val _ = D.printDebug 3 D.JSON ("getting basis...")
	val bas = getInt(basis)
	val _ = D.printDebug 3 D.JSON ("getting timelimit...")
	val tm = getIntInf(timelimit)
	val _ = D.printDebug 3 D.JSON ("getting labelling...")
	val lbing = getString(labelling)
	val _ = D.printDebug 3 D.JSON ("getting final...")
	val fin = getBool(final)
	val _ = D.printDebug 3 D.JSON ("getting name...")
	val nm = getString(name)
	val _ = D.printDebug 3 D.JSON ("got everything, new getting time...")
    in
	ref (SOME {
	     errors = errs,
