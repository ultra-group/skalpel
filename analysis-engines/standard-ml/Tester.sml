...
(*
fun initSlicing filebas filesin nenv webdemo =
    let val _ = resetAll ()
	...
	(*val parse as (ast, m, (asc, ascid)) = P.parse (filein, stmin) initlab
	...
	(*val _ = TextIO.closeIn stmin*)
        ...
        val (errl1, filters1) = EN.preEnum envcss
	val timerEnum = VT.startTimer ()
	val (errl2, filters2) = EN.initEnum env errl1 filters1 timerEnum progs
        val errl3 = ERR.setSlices progs errl2
	val errl4 = ERR.setRegs errl3 true
    in (initlab, parse, envcss, errl4, filters2, timerEnum)
    end

fun runSlicing (parse as (ast, m, ascid))
	       (envcss as (env, css))
	       found
	       filters
	       done
	       timerEnum =
    let val (errs1, found', filters', done') = EN.runEnum env found filters done (gettimelimit ()) timerEnum ast
	val found'' = ERR.recordSpeTreat found'
	val _ = if List.length found' > List.length found''
		then raise EH.DeadBranch ""
		else ()
	val errs2   = case errs1 of
			  []    => []
			| [err] => ERR.getMergedErrors (err :: ERR.getNewErrors found' found'')
			| _     => raise EH.DeadBranch ""
        val errs3 = ERR.setSlices ast errs2
	val errs4 = ERR.setRegs errs3 true
    in case errs1 of
	   [] => NONE
	 | _  => SOME (errs4, found'', filters', done')
    end

fun loopSlicing filebas filesin funout nenv webdemo =
    let val (initlab, parse, envcss, errs, filters, timerEnum) =
	    initSlicing filebas filesin nenv webdemo
	val counter = ref 0
	val bmin    = false
  	val done    = []
	val tdummy  = Int.toLarge 0
	val times   = (tdummy, tdummy, tdummy, tdummy, tdummy)
	val name    = "dummy"
	(* Treatment of the first errors if any *)
	fun treatInit [] = ()
	  | treatInit (err :: errs) =
	    (counter := !counter + 1;
	     print (Int.toString (!counter) ^ " :" ^
		    Int.toString (Int.fromLarge (VT.getMilliTime timerEnum)) ^ "\n");
	     funout [err] parse bmin times envcss initlab name "" (!counter);
	     treatInit errs)
	fun run found filters done =
	    case runSlicing parse envcss found filters done timerEnum of
		NONE => !counter
	      | SOME (errs', found', filters', done') =>
		(counter := !counter + 1;
		 print (Int.toString (!counter) ^ " " ^
			Int.toString (List.length errs') ^ " :" ^
			Int.toString (Int.fromLarge (VT.getMilliTime timerEnum)) ^ "\n");
		 funout errs' parse bmin times envcss initlab name "" (!counter);
		 run found' filters' done')
	val _ = treatInit errs
    in case filters of
	   [] => !counter
	 | _  => run errs filters done
    end
*)
