...
*** (2012-06-28-16:00) Generating equality constraints for tuples
...
*** (2012-08-08-15:42) Records

    The solution for the implementation of detecting equality type
    errors in records should be similar to detecting equality type
    errors in tuples. A tuple is just a record that has an enumeration
    sequence on the left hand side.

*** (2012-08-08-16:13) Value declarations with tics
...
- val 'b f = fn x : 'b => x = x; fragment untypable, but 'a in
  datatype constructors allowed to be used with equalop? Other cases
  of this?
- equality type propagation with type x : int, where eqtype int, but
  type y : real, where type real. Functions not specified either?
  Check and ask.
- testing of copyright statements
