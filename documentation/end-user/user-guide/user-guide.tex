...
\item \texttt{skalpel -e <0 | 1>} Setting the -e parameter to 0 or 1
  ...
  the terminal. This should always be set to 1. \todo{maybe there
    should be an environment variable for this?}
\end{itemize}

\todo[inline]{An example should go here of the Skalpel terminal front
  end being executed}
...
\noindent \texttt{(add-to-list 'load-path (file-truename "/path/to/extracted/archive/EmacsUI/"))}
\texttt{(defvar skalpel-emacs-directory "/path/to/extracted/archive/EmacsUI/")}
...
Emacs should now be configured to use Skalpel on Standard ML programs.
