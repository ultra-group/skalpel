...
echo "Errors:" >> $mailFile
cat $compilationLog-errors >> $mailFile
...
then
    echo "no compilation log errors, removing empty errors file..."
...
if [ ! -n "$analysisErrors" ]
then
    echo "no errors in the execution of analysis engine tests, removing empty errors file..."
    rm -f "$outputDir/$analysisTestFilename-errors"
fi

# we check if the dead links test file exists, this is generated by another script on
# the webserver, so we need to make sure this really exists first (it certainly won't
# if you're testing this script and executing it manually)
if [ -e "$outputDir/$deadLinksTestFilename-errors" ]
then
    deadLinksErrors=`cat $outputDir/$deadLinksTestFilename-errors`
    if [ ! -n "$deadLinksErrors" ]
    then
	echo "no errors in the execution of checking for dead links of the web page, removing empty errors file..."
	rm "-f $outputDir/$deadLinksTestFilename-errors"
    fi
fi
